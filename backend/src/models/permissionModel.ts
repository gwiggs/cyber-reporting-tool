...
// Type for permission creation data
...
// Type for permission update data
...
const permissionModel = {
  ...
  async create(data: CreatePermissionData): Promise<Permission> {
    const result = await db.query<Permission>(
      `INSERT INTO permissions (name, description, resource, action)
       VALUES ($1, $2, $3, $4)
       RETURNING *`,
      [data.name, data.description || null, data.resource, data.action]
    );
   ...
   * Update a permission
   */
  async update(id: number, data: UpdatePermissionData): Promise<Permission | null> {
   ...
   */
  async getRolesWithPermission(permissionId: number): Promise<number[]> {
    const result = await db.query<{ role_id: number }>(
