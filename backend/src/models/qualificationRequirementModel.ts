...
const qualificationRequirementModel = {
   ...
   */
  async findAll(): Promise<QualificationRequirement[]> {
    const result = await db.query<QualificationRequirement & { qualification_name: string, work_role_name: string }>(
      `SELECT qr.*, q.name as qualification_name, wr.name as work_role_name
   ...
   */
  async findById(id: number): Promise<QualificationRequirement | null> {
    const result = await db.query<QualificationRequirement & { qualification_name: string, work_role_name: string }>(
      `SELECT qr.*, q.name as qualification_name, wr.name as work_role_name
       FROM qualification_requirements qr
       JOIN qualifications q ON qr.qualification_id = q.id
   ...
   */
  async findByWorkRoleId(workRoleId: number): Promise<QualificationRequirement[]> {
    const result = await db.query<QualificationRequirement & { qualification_name: string }>(
      `SELECT qr.*, q.name as qualification_name
       FROM qualification_requirements qr
       JOIN qualifications q ON qr.qualification_id = q.id
       WHERE qr.work_role_id = $1
       ORDER BY qr.priority DESC, q.name`,
      [workRoleId]
    );
    return result.rows;
  },
   ...
   */
  async findWorkRolesRequiringQualification(qualificationId: number): Promise<QualificationRequirement[]> {
    const result = await db.query<QualificationRequirement & { work_role_name: string }>(
      `SELECT qr.*, wr.name as work_role_name
       FROM qualification_requirements qr
       JOIN work_roles wr ON qr.work_role_id = wr.id
       WHERE qr.qualification_id = $1
       ORDER BY qr.is_required DESC, qr.priority DESC`,
      [qualificationId]
    );
    return result.rows;
  },
   ...
   */
  async checkIsQualificationRequired(workRoleId: number, qualificationId: number): Promise<boolean> {
    const result = await db.query<{ is_required: boolean }>(
      `SELECT is_required FROM qualification_requirements
       WHERE work_role_id = $1 AND qualification_id = $2`,
      [workRoleId, qualificationId]
    );
   ...
   */
  async create(data: CreateQualificationRequirementData): Promise<QualificationRequirement> {
    return db.transaction(async (client: PoolClient) => {
      // Check if the requirement already exists
      const existingResult = await client.query(
        `SELECT id FROM qualification_requirements 
         WHERE work_role_id = $1 AND qualification_id = $2`,
        [data.work_role_id, data.qualification_id]
      );
      ...
      const result = await client.query<QualificationRequirement>(
   ...
   */
  async update(id: number, data: UpdateQualificationRequirementData): Promise<QualificationRequirement> {
    ...
    const result = await db.query<QualificationRequirement>(
  ...
  async delete(id: number): Promise<void> {
    await db.query(
      'DELETE FROM qualification_requirements WHERE id = $1',
      [id]
    );
  },
   ...
   */
  async findMissingQualificationsForUser(userId: number): Promise<any[]> {
    const result = await db.query(
      `WITH user_current_roles AS (
       ...
       ORDER BY rq.priority DESC, rq.qualification_name`,
      [userId]
    );
   ...
   */
  async checkUserQualificationsForWorkRole(userId: number, workRoleId: number): Promise<{ 
    qualified: boolean, 
    missingRequired: number, 
    missingRecommended: number 
  }> {
    const result = await db.query(
      `WITH required_quals AS (
...
export default qualificationRequirementModel; 