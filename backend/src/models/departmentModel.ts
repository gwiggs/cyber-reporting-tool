...
// Type for department creation data
...
// Type for department update data
...
const departmentModel = {
  ...
  async create(data: CreateDepartmentData): Promise<Department> {
    const result = await db.query<Department>(
      `INSERT INTO departments (organization_id, name, department_code)
       VALUES ($1, $2, $3)
       RETURNING *`,
      [data.organization_id, data.name, data.department_code || null]
    );
   ...
   * Update a department
   */
  async update(id: number, data: UpdateDepartmentData): Promise<Department | null> {
   ...
   */
  async findByCode(organizationId: number, code: string): Promise<Department | null> {
    const result = await db.query<Department>(
      'SELECT * FROM departments WHERE organization_id = $1 AND department_code = $2',
      [organizationId, code]
    );
    return result.rows[0] || null;
  },
   ...
   */
  async getUsersCount(departmentId: number): Promise<number> {
