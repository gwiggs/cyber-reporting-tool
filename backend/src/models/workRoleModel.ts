...
const workRoleModel = {
  ...
  async findAll(): Promise<WorkRole[]> {
    const result = await db.query<WorkRole & { department_name: string }>(
      `SELECT wr.*, d.name as department_name
       FROM work_roles wr
   ...
   */
  async findById(id: number): Promise<WorkRole | null> {
    const result = await db.query<WorkRole & { department_name: string }>(
      `SELECT wr.*, d.name as department_name
       FROM work_roles wr
       LEFT JOIN departments d ON wr.department_id = d.id
   ...
   */
  async findByCode(code: string): Promise<WorkRole | null> {
    const result = await db.query<WorkRole>(
      'SELECT * FROM work_roles WHERE code = $1',
      [code]
    );
    return result.rows[0] || null;
  },
   ...
   * Create work role
   */
  async create(data: CreateWorkRoleData): Promise<WorkRole> {
    const result = await db.query<WorkRole>(
   ...
   * Update work role
   */
  async update(id: number, data: UpdateWorkRoleData): Promise<WorkRole> {
  ...
  async delete(id: number): Promise<void> {
    await db.query(
      'DELETE FROM work_roles WHERE id = $1',
      [id]
    );
  },
   ...
   */
  async findByDepartment(departmentId: number): Promise<WorkRole[]> {
    const result = await db.query<WorkRole>(
      'SELECT * FROM work_roles WHERE department_id = $1 ORDER BY name',
      [departmentId]
    );
    return result.rows;
  },
   ...
   */
  async findAllUserWorkRoles(): Promise<UserWorkRole[]> {
    const result = await db.query<UserWorkRole & { work_role_name: string, user_name: string }>(
      `SELECT uwr.*, wr.name as work_role_name, 
   ...
   */
  async findUserWorkRolesByUserId(userId: number): Promise<UserWorkRole[]> {
    const result = await db.query<UserWorkRole & { work_role_name: string }>(
      `SELECT uwr.*, wr.name as work_role_name
       FROM user_work_roles uwr
       JOIN work_roles wr ON uwr.work_role_id = wr.id
       WHERE uwr.user_id = $1
       ORDER BY uwr.primary_role DESC, uwr.start_date DESC`,
      [userId]
    );
    return result.rows;
  },
   ...
   */
  async findUsersByWorkRole(workRoleId: number): Promise<UserWorkRole[]> {
    const result = await db.query<UserWorkRole & { user_name: string }>(
      `SELECT uwr.*, u.first_name || ' ' || u.last_name as user_name
       FROM user_work_roles uwr
       JOIN users u ON uwr.user_id = u.id
   ...
   */
  async assignWorkRoleToUser(data: CreateUserWorkRoleData): Promise<UserWorkRole> {
    return db.transaction(async (client: PoolClient) => {
      // Check if the assignment already exists
      const existingResult = await client.query(
        `SELECT * FROM user_work_roles 
         WHERE user_id = $1 AND work_role_id = $2 AND 
         (end_date IS NULL OR end_date > CURRENT_DATE)`,
        [data.user_id, data.work_role_id]
      );
      ...
      if (data.primary_role) {
        await client.query(
          `UPDATE user_work_roles 
           SET primary_role = false, updated_at = CURRENT_TIMESTAMP
           WHERE user_id = $1 AND primary_role = true`,
          [data.user_id]
        );
      }

      // Insert the new work role assignment
      const result = await client.query<UserWorkRole>(
        `INSERT INTO user_work_roles
   ...
   */
  async updateUserWorkRole(userId: number, workRoleId: number, data: UpdateUserWorkRoleData): Promise<UserWorkRole> {
    return db.transaction(async (client: PoolClient) => {
      ...
      if (data.primary_role) {
        await client.query(
          `UPDATE user_work_roles 
           SET primary_role = false, updated_at = CURRENT_TIMESTAMP
           WHERE user_id = $1 AND work_role_id != $2 AND primary_role = true`,
          [userId, workRoleId]
        );
      }
   ...
   */
  async endWorkRoleAssignment(userId: number, workRoleId: number, endDate: Date): Promise<UserWorkRole> {
    const result = await db.query<UserWorkRole>(
      `UPDATE user_work_roles
       SET end_date = $1, updated_at = CURRENT_TIMESTAMP
       WHERE user_id = $2 AND work_role_id = $3
       RETURNING *`,
      [endDate, userId, workRoleId]
    );
   ...
   */
  async getUserPrimaryWorkRole(userId: number): Promise<UserWorkRole | null> {
    const result = await db.query<UserWorkRole & { work_role_name: string }>(
      `SELECT uwr.*, wr.name as work_role_name
       FROM user_work_roles uwr
       JOIN work_roles wr ON uwr.work_role_id = wr.id
   ...
   */
  async getUserCurrentWorkRoles(userId: number): Promise<UserWorkRole[]> {
    const result = await db.query<UserWorkRole & { work_role_name: string }>(
      `SELECT uwr.*, wr.name as work_role_name
       FROM user_work_roles uwr
       JOIN work_roles wr ON uwr.work_role_id = wr.id
       WHERE uwr.user_id = $1
       AND (uwr.end_date IS NULL OR uwr.end_date > CURRENT_DATE)
       ORDER BY uwr.primary_role DESC, wr.name`,
      [userId]
    );
    return result.rows;
  },
   ...
   */
  async getUserHistoricalWorkRoles(userId: number): Promise<UserWorkRole[]> {
    const result = await db.query<UserWorkRole & { work_role_name: string }>(
      `SELECT uwr.*, wr.name as work_role_name
       FROM user_work_roles uwr
       JOIN work_roles wr ON uwr.work_role_id = wr.id
