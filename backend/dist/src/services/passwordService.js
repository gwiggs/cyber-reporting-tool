...
const passwordService = {
    ...
    generateRandomPassword(options = {}) {
        ...
        // Generate password
        let password = '';
        const randomBytes = crypto_1.default.randomBytes(config.length * 2); // Get more bytes than needed for better randomness
        ...
        if (config.includeUppercase && !/[A-Z]/.test(finalPassword)) {
            const randomPos = crypto_1.default.randomInt(0, finalPassword.length);
            const randomChar = uppercaseChars[crypto_1.default.randomInt(0, uppercaseChars.length)];
        ...
        if (config.includeLowercase && !/[a-z]/.test(finalPassword)) {
            const randomPos = crypto_1.default.randomInt(0, finalPassword.length);
            const randomChar = lowercaseChars[crypto_1.default.randomInt(0, lowercaseChars.length)];
            finalPassword = finalPassword.substring(0, randomPos) + randomChar + finalPassword.substring(randomPos + 1);
        }
        if (config.includeNumbers && !/[0-9]/.test(finalPassword)) {
            const randomPos = crypto_1.default.randomInt(0, finalPassword.length);
            const randomChar = numberChars[crypto_1.default.randomInt(0, numberChars.length)];
            finalPassword = finalPassword.substring(0, randomPos) + randomChar + finalPassword.substring(randomPos + 1);
        }
        if (config.includeSpecials && !/[!@#$%^&*()_\-+=<>?]/.test(finalPassword)) {
            const randomPos = crypto_1.default.randomInt(0, finalPassword.length);
            const randomChar = specialChars[crypto_1.default.randomInt(0, specialChars.length)];
            finalPassword = finalPassword.substring(0, randomPos) + randomChar + finalPassword.substring(randomPos + 1);
        }
