...
const permissionAssignmentSchema = zod_1.z.object({
...
const roleController = {
    ...
    async create(req, res) {
        try {
            ...
            const existingRole = await roleModel_1.default.findByName(validatedData.name);
            if (existingRole) {
                res.status(400).json({
                    success: false,
            ...
            // Create role
            const role = await roleModel_1.default.create(validatedData);
            // Return created role
            res.status(201).json({
                success: true,
                data: role,
    ...
    async getAllPublic(req, res) {
        try {
            ...
            const { roles } = await roleModel_1.default.findAll(100, 0, 'name', 'ASC');
            res.status(200).json({
                success: true,
    ...
    async getPermissions(req, res) {
        try {
            ...
            const role = await roleModel_1.default.findById(id);
            if (!role) {
                res.status(404).json({
                    success: false,
            ...
            // Get permissions
            const permissions = await roleModel_1.default.getRolePermissions(id);
            res.status(200).json({
                success: true,
                data: permissions
            });
    ...
    async getAll(req, res) {
        try {
            ...
            // Get roles
            const { roles, total } = await roleModel_1.default.findAll(limit, offset, orderBy, direction);
    ...
    async getById(req, res) {
        try {
            ...
            const usersCount = await roleModel_1.default.getUsersWithRole(id);
            res.status(200).json({
                success: true,
                data: {
                    ...role,
    ...
    async update(req, res) {
        try {
            ...
            // Check if role exists
            const existingRole = await roleModel_1.default.findById(id);
            if (!existingRole) {
                res.status(404).json({
                    success: false,
                    message: 'Role not found',
            ...
            if (validatedData.name && validatedData.name !== existingRole.name) {
                const nameExists = await roleModel_1.default.findByName(validatedData.name);
                if (nameExists) {
                    res.status(400).json({
                        success: false,
                        message: 'A role with this name already exists',
            ...
            // Update role
            const updatedRole = await roleModel_1.default.update(id, validatedData);
            res.status(200).json({
                success: true,
                data: updatedRole,
            });
    ...
    async delete(req, res) {
        try {
            ...
            // Delete role
            const result = await roleModel_1.default.delete(id);
            if (!result) {
                res.status(400).json({
                    success: false,
    ...
    async addPermission(req, res) {
        try {
            ...
            const permissionModel = (await Promise.resolve().then(() => __importStar(require('../models/permissionModel')))).default;
            ...
            // Add permission to role
            await roleModel_1.default.addPermission(roleId, permission_id);
            // Get updated permissions
            const permissions = await roleModel_1.default.getRolePermissions(roleId);
            res.status(200).json({
                success: true,
    ...
    async removePermission(req, res) {
        try {
            ...
            // Remove permission from role
            const result = await roleModel_1.default.removePermission(roleId, permissionId);
            if (!result) {
                res.status(404).json({
                    success: false,
